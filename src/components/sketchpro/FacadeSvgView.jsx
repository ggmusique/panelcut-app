...
export default function FacadeSvgView({
  ...
  const cmToSvgY = (cm, mRect) =>
    mRect.y + (1 - Math.min(1, Math.max(0, cm / cabinetH))) * mRect.height;
  ...
  const handleMouseDown = useCallback((e) => {
    ...
    if (['shelf','drawer','rod','door','sliding_door'].includes(tool)) {
  ...
  // Icône poubelle sobre — visible uniquement au hover
  const TrashIcon = ({ x, y, onClick, visible }) => (
    <g onClick={onClick} style={{ cursor: 'pointer', opacity: visible ? 1 : 0, transition: 'opacity 0.15s' }}
      pointerEvents={visible ? 'all' : 'none'}>
      <rect x={x-11} y={y-11} width={22} height={22} rx={4} fill="#1e293b" stroke="#475569" strokeWidth="1" />
      <rect x={x-5} y={y-3} width={10} height={8} rx={1} fill="none" stroke="#94a3b8" strokeWidth={1.3} />
  ...
  return (
    <svg ref={svgRef} width="100%" height="100%"
      ...
      <defs>
        ...
        {/* Tringle chromée — tube cylindrique réaliste */}
        <linearGradient id="gRodTube" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stopColor="#ffffff" />
          <stop offset="18%"  stopColor="#d0d8e0" />
          <stop offset="42%"  stopColor="#788090" />
          <stop offset="58%"  stopColor="#c8d0d8" />
          <stop offset="80%"  stopColor="#909aa4" />
          <stop offset="100%" stopColor="#505860" />
        </linearGradient>
        <linearGradient id="gRodBracket" x1="0" y1="0" x2="1" y2="1">
      ...
      {layout.moduleRects.length > 0 && (() => {
        ...
        return (
          <g>
            <rect x={bx+th} y={by+th} width={bw-2*th} height={bh-th} fill="url(#gBack)" />
            <rect x={bx} y={by} width={th} height={bh} fill="url(#gWoodFace)" stroke="#c0980080" strokeWidth={0.5} />
            <rect x={bx+th} y={by+th} width={6} height={bh-th} fill="url(#gWoodInner)" opacity={0.5} />
            <rect x={bx+bw-th} y={by} width={th} height={bh} fill="url(#gWoodFace)" stroke="#c0980080" strokeWidth={0.5} />
            <rect x={bx+bw-th-6} y={by+th} width={6} height={bh-th} fill="url(#gWoodInner)" opacity={0.5} />
            <rect x={bx} y={by} width={bw} height={th} fill="url(#gWoodFace)" stroke="#c0980080" strokeWidth={0.5} />
            <rect x={bx+th} y={by+th} width={bw-2*th} height={5} fill="#00000018" />
            <rect x={bx} y={by+bh-th} width={bw} height={th} fill="url(#gWoodFace)" stroke="#c0980080" strokeWidth={0.5} />
            {pl > 0 && <rect x={bx+th*1.5} y={by+bh} width={bw-th*3} height={pl} fill="url(#gWoodFace)" stroke="#c0980060" strokeWidth={0.5} />}
          </g>
        );
      })()}
      ...
      {layout.moduleRects.map((mRect, mIdx) => {
        ...
        return (
          <g key={mRect.id}>
            ...
            {/* ── DOUBLE MONTANT entre modules ── */}
            {mIdx > 0 && (() => {
              const sx = mRect.x;
              const sy = mRect.y;
              const sh = mRect.height;
              return (
                <g>
                  {/* Montant gauche du nouveau module */}
                  <rect x={sx} y={sy} width={th} height={sh}
                    fill="url(#gWoodFace)" stroke="#c0980070" strokeWidth={0.5} />
                  {/* Léger espace sombre entre les 2 montants (joint) */}
                  <rect x={sx + th - 1} y={sy} width={2} height={sh}
                    fill="#00000025" />
                  {/* Montant droit du module précédent (déjà dessiné via structure, on renforce) */}
                  <rect x={sx - th} y={sy} width={th} height={sh}
                    fill="url(#gWoodFace)" stroke="#c0980070" strokeWidth={0.5} />
                </g>
              );
            })()}
            ...
            {/* Numéro cerclé */}
            <circle cx={ix+iw/2} cy={iy+26} r={14} fill="white" stroke="#d43010" strokeWidth={1.5} />
            <text x={ix+iw/2} y={iy+31} textAnchor="middle" fill="#d43010"
              fontSize={12} fontWeight="700" fontFamily="Arial, sans-serif">{mRect.id}</text>
            ...
            {(md.rods || []).map((r) => {
              ...
              const rodR = 5; // rayon apparent du tube
              const bracketW = 10;
              const bracketH = 28;
              return (
                <g key={r.id}
                  ...
                  {/* Zone drag */}
                  <rect x={ix} y={ry-rodR-14} width={iw} height={rodR*2+28}
                    fill="transparent" style={{ cursor: 'ns-resize' }}
                    onMouseDown={(e) => startDrag(e, mRect.id, 'rods', r.id)} />

                  {/* Support gauche — plaque vissée au panneau */}
                  <rect x={ix+6} y={ry-bracketH/2} width={bracketW} height={bracketH} rx={3}
                    fill="url(#gRodBracket)" stroke="#50606870" strokeWidth={0.8} pointerEvents="none" />
                  {/* Vis support gauche */}
                  <circle cx={ix+11} cy={ry-bracketH/2+5} r={2} fill="#303840" stroke="#606870" strokeWidth={0.5} pointerEvents="none" />
                  <circle cx={ix+11} cy={ry+bracketH/2-5} r={2} fill="#303840" stroke="#606870" strokeWidth={0.5} pointerEvents="none" />
                  {/* Crochet support gauche */}
                  <path d={`M${ix+16} ${ry} Q${ix+20} ${ry} ${ix+20} ${ry+rodR+2}`}
                    fill="none" stroke="#788090" strokeWidth={2.5} strokeLinecap="round" pointerEvents="none" />

                  {/* Support droit — plaque vissée */}
                  <rect x={ix+iw-bracketW-6} y={ry-bracketH/2} width={bracketW} height={bracketH} rx={3}
                    fill="url(#gRodBracket)" stroke="#50606870" strokeWidth={0.8} pointerEvents="none" />
                  <circle cx={ix+iw-11} cy={ry-bracketH/2+5} r={2} fill="#303840" stroke="#606870" strokeWidth={0.5} pointerEvents="none" />
                  <circle cx={ix+iw-11} cy={ry+bracketH/2-5} r={2} fill="#303840" stroke="#606870" strokeWidth={0.5} pointerEvents="none" />
                  <path d={`M${ix+iw-16} ${ry} Q${ix+iw-20} ${ry} ${ix+iw-20} ${ry+rodR+2}`}
                    fill="none" stroke="#788090" strokeWidth={2.5} strokeLinecap="round" pointerEvents="none" />

                  {/* Tube principal — ellipse pour simuler la rondeur */}
                  <rect x={ix+20} y={ry-rodR} width={iw-40} height={rodR*2} rx={rodR}
                    fill="url(#gRodTube)" stroke="#60707880" strokeWidth={0.5} pointerEvents="none" />
                  {/* Reflet principal (bande lumineuse en haut) */}
                  <rect x={ix+24} y={ry-rodR+1} width={iw-48} height={rodR*0.55} rx={rodR*0.4}
                    fill="rgba(255,255,255,0.72)" pointerEvents="none" />
                  {/* Reflet secondaire bas */}
                  <rect x={ix+28} y={ry+rodR*0.3} width={iw-56} height={rodR*0.3} rx={2}
                    fill="rgba(255,255,255,0.28)" pointerEvents="none" />
                  {/* Ombre portée sous tube */}
                  <ellipse cx={ix+iw/2} cy={ry+rodR+2} rx={(iw-40)*0.45} ry={2}
                    fill="#00000020" pointerEvents="none" />
                  ...
                  <TrashIcon x={ix+iw-14} y={ry-rodR-14} visible={isHov}
            ...
            {(md.shelves || []).map((s) => {
              ...
              return (
                <g key={s.id}
                  ...
                  onMouseLeave={() => setHoveredItem(null)}>
                  <rect x={ix} y={sy2-shH-8} width={iw} height={shH+16} fill="transparent"
                    style={{ cursor: 'ns-resize' }}
                    onMouseDown={(e) => startDrag(e, mRect.id, 'shelves', s.id)} />
                  <rect x={ix} y={sy2-shH} width={iw} height={shH}
                    fill="url(#gShelf)" filter="url(#fShadow)" pointerEvents="none" />
                  <rect x={ix} y={sy2-shH} width={iw} height={2.5} fill="#a07828" pointerEvents="none" />
                  <rect x={ix+4} y={sy2-shH+3} width={iw*0.4} height={1.5} fill="rgba(255,255,255,0.5)" pointerEvents="none" />
                  <rect x={ix} y={sy2} width={iw} height={3} fill="#00000018" pointerEvents="none" />
                  {isDrag && <line x1={mRect.x} x2={mRect.x+mRect.width} y1={sy2-shH/2} y2={sy2-shH/2}
                    stroke="#d43010" strokeWidth={1} strokeDasharray="5 3" pointerEvents="none" />}
                  <TrashIcon x={ix+iw-14} y={sy2-shH-8} visible={isHov}
            ...
            {(md.drawers || []).map((d) => {
              ...
              return (
                <g key={d.id}
                  ...
                  <rect x={ix+1} y={topY} width={iw-2} height={2.5} fill="#9a7828" pointerEvents="none" />
                  <rect x={ix+1} y={topY+dh-2} width={iw-2} height={2} fill="#9a782870" pointerEvents="none" />
                  <rect x={ix+6} y={topY+4} width={iw*0.35} height={1.5} fill="rgba(255,255,255,0.55)" pointerEvents="none" />
                  ...
                  <rect x={ix+iw/2-18} y={topY+dh/2-1} width={36} height={1.5} fill="rgba(255,255,255,0.6)" pointerEvents="none" />
                  ...
                  <TrashIcon x={ix+iw-14} y={topY+8} visible={isHov}
            ...
            {(md.slidingDoors || []).length > 0 && (() => {
              ...
              return (
                <g onMouseEnter={() => setHoveredItem({ moduleId: mRect.id, collection: 'slidingDoors', itemId: md.slidingDoors[0].id })}
                  onMouseLeave={() => setHoveredItem(null)}>
                  <rect x={ix} y={iy} width={iw} height={4} fill="#b0b8c0" rx={1} />
                  <rect x={ix} y={iy+ih-4} width={iw} height={4} fill="#b0b8c0" rx={1} />
                  <rect x={ix+iw*0.44} y={iy+4} width={iw*0.54} height={ih-8} fill="url(#gGlass)" stroke="#90a8c0" strokeWidth={1.2} />
                  <rect x={ix+iw*0.46} y={iy+10} width={2} height={ih-20} fill="rgba(255,255,255,0.6)" />
                  <rect x={ix+2} y={iy+4} width={iw*0.54} height={ih-8} fill="url(#gGlass)" stroke="#90a8c0" strokeWidth={1.2} opacity={0.9} />
                  <rect x={ix+8} y={iy+10} width={2} height={ih-20} fill="rgba(255,255,255,0.6)" />
                  <rect x={ix+iw*0.44-4} y={iy+ih/2-18} width={5} height={36} rx={2.5} fill="url(#gHandle)" stroke="#80808880" strokeWidth={0.6} />
                  <rect x={ix+iw*0.56-1} y={iy+ih/2-18} width={5} height={36} rx={2.5} fill="url(#gHandle)" stroke="#80808880" strokeWidth={0.6} />
                  <TrashIcon x={ix+iw-14} y={iy+10} visible={isHov}
            ...
            {(md.slidingDoors||[]).length === 0 && (md.doors||[]).map((d, idx) => {
              ...
              return (
                <g key={d.id}
                  ...
                  onMouseLeave={() => setHoveredItem(null)}>
                  <rect x={dx+1} y={iy+1} width={dw-2} height={ih-2} fill="url(#gDoor)" stroke="#b09030" strokeWidth={1} />
                  <rect x={dx+7} y={iy+9} width={dw-14} height={ih-18} fill="none" stroke="#c0a04080" strokeWidth={1} />
                  <rect x={dx+4} y={iy+3} width={2} height={ih-6} fill="rgba(255,255,255,0.28)" />
                  {[0.22,0.78].map((f,ci) => (
                    <rect key={ci} x={isLeft ? dx+dw-5 : dx} y={iy+ih*f-8} width={5} height={16} rx={1}
                      fill="#a0a8b0" stroke="#70787880" strokeWidth={0.5} />
                  ))}
                  <rect x={isLeft ? dx+8 : dx+dw-13} y={iy+ih/2-20} width={5} height={40} rx={2.5}
            ...
            {/* Cote largeur module */}
            <line x1={mRect.x+2} y1={oy+innerH+18} x2={mRect.x+mRect.width-2} y2={oy+innerH+18}
              stroke="#d42020" strokeWidth={0.9} markerStart="url(#mDim)" markerEnd="url(#mDim)" />
            ...
            <text x={mRect.x+mRect.width/2} y={oy+innerH+33} textAnchor="middle"
              fill="#d42020" fontSize={9} fontWeight="600" fontFamily="Arial, sans-serif">{fmWidth} cm</text>
          </g>
        );
      })}

      {/* Cotes générales */}
      <line x1={ox} y1={oy-32} x2={ox+innerW} y2={oy-32} stroke="#d42020" strokeWidth={1}
      ...
      <rect x={ox+innerW/2-26} y={oy-46} width={52} height={16} rx={3} fill="white" stroke="#d42020" strokeWidth={0.8} />
      <text x={ox+innerW/2} y={oy-34} textAnchor="middle" fill="#d42020" fontSize={11} fontWeight="700" fontFamily="Arial, sans-serif">
        {draftState?.cabinetDims?.width || '?'} cm
      </text>

      <line x1={ox+innerW+32} y1={oy} x2={ox+innerW+32} y2={oy+innerH-pl} stroke="#d42020" strokeWidth={1}
        markerStart="url(#mDim)" markerEnd="url(#mDim)" />
      ...
      <g transform={`rotate(-90, ${ox+innerW+54}, ${oy+(innerH-pl)/2})`}>
        <rect x={ox+innerW+54-26} y={oy+(innerH-pl)/2-8} width={52} height={16} rx={3} fill="white" stroke="#d42020" strokeWidth={0.8} />
        <text x={ox+innerW+54} y={oy+(innerH-pl)/2+5} textAnchor="middle" fill="#d42020" fontSize={11} fontWeight="700" fontFamily="Arial, sans-serif">
      ...
      {(draftState?.facadeItems || []).map((a) => {
        ...
        if (a.type === 'dim') return (
          <g key={a.id} onClick={() => onAnnotationClick?.(a.id)} style={{ cursor: 'pointer' }}>
            ...
            <text x={(a.x1+a.x2)/2} y={(a.y1+a.y2)/2+4} textAnchor="middle"
              fill={sel ? '#f97316' : '#d42020'} fontSize={9} fontWeight="700" fontFamily="Arial, sans-serif">{a.label||'cote'}</text>
          </g>
        );
        ...
        return (
          <g key={a.id} onClick={() => onAnnotationClick?.(a.id)} style={{ cursor: 'pointer' }}>
            ...
            <text x={a.x1} y={a.y1+3} textAnchor="middle" fill="#0369a1" fontSize={9} fontWeight="600" fontFamily="Arial, sans-serif">{a.label||'note'}</text>
          </g>
        );
      })}
