...
export default function FacadeSvgView({
  ...
  // Convertit un event souris en coordonnées SVG internes (espace viewBox 860×450)
  ...
  const handleMouseDown = (e) => {
    ...
    if (objectTools.includes(tool)) {
      ...
      if (!m) return;
      // Convertit la position Y pixel SVG → cm selon la hauteur cabinet
      const cabinetH = Math.max(1, Number(draftState?.cabinetDims?.height || 220));
      const yCm = ((p.y - m.y) / Math.max(1, m.height)) * cabinetH;
      onAddObject?.(m.id, tool, Math.max(0, Math.min(cabinetH, yCm)));
      return;
    }
  ...
  const cursor = tool === 'erase' ? 'not-allowed'
  ...
  return (
    ...
    >
      ...
      {layout.moduleRects.map((mRect) => {
        ...
        // cmToY : convertit une hauteur cm (depuis le bas) en pixel SVG
        const cmToY = (cm) => moduleTop + (1 - cm / cabinetH) * mRect.height;
        ...
        return (
          <g key={mRect.id} onClick={() => onModuleClick?.(mRect.id)} style={{ cursor: 'pointer' }}>
            ...
            {(md.rods || []).map((r) => {
              const ry = cmToY(Number(r.y) || cabinetH * 0.88);
              ...
              return (
                <g key={r.id} onClick={(e) => { e.stopPropagation(); onItemClick?.(mRect.id, 'rod', r.id); }}>
                  ...
                  <rect
                    x={mRect.x + mRect.width - 22} y={ry - 10} width="18" height="18" rx="4"
                    fill="#ef4444" opacity="0.85"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'rods', r.id); }}
                    style={{ cursor: 'pointer' }}
                  />
                  <text x={mRect.x + mRect.width - 13} y={ry + 5} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'rods', r.id); }}
            ...
            {(md.shelves || []).map((s) => {
              const sy = cmToY(Number(s.y) || 0);
              const active = selectedItemId === s.id;
              return (
                <g key={s.id} onClick={(e) => { e.stopPropagation(); onItemClick?.(mRect.id, 'shelf', s.id); }}>
                  <rect x={mRect.x + 3} y={sy - 2} width={mRect.width - 6} height="4"
                    fill="#9ca3af" stroke={active ? '#f97316' : '#6b7280'} strokeWidth={active ? 1.6 : 0.8}
                  />
                  <rect
                    x={mRect.x + mRect.width - 22} y={sy - 10} width="18" height="18" rx="4"
                    fill="#ef4444" opacity="0.85"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'shelves', s.id); }}
                    style={{ cursor: 'pointer' }}
                  />
                  <text x={mRect.x + mRect.width - 13} y={sy + 5} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'shelves', s.id); }}
                    style={{ cursor: 'pointer', pointerEvents: 'none' }}
                  >×</text>
                </g>
            ...
            {(md.drawers || []).map((d) => {
              ...
              const topY = cmToY(yCm + hCm);
              const dh   = Math.max(10, cmToY(yCm) - topY);
              const active = selectedItemId === d.id;
              return (
                <g key={d.id} onClick={(e) => { e.stopPropagation(); onItemClick?.(mRect.id, 'drawer', d.id); }}>
                  <rect x={mRect.x + 4} y={topY} width={mRect.width - 8} height={dh}
                    fill="#f5ede0" stroke={active ? '#f97316' : '#8b6914'} strokeWidth={active ? 1.8 : 1} rx="2" />
                  <ellipse cx={mRect.x + mRect.width / 2} cy={topY + dh / 2} rx="12" ry="4" fill="#9ca3af" />
                  <rect
                    x={mRect.x + mRect.width - 22} y={topY + 2} width="18" height="18" rx="4"
                    fill="#ef4444" opacity="0.85"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'drawers', d.id); }}
                    style={{ cursor: 'pointer' }}
                  />
                  <text x={mRect.x + mRect.width - 13} y={topY + 15} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold"
                    onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'drawers', d.id); }}
                    style={{ cursor: 'pointer', pointerEvents: 'none' }}
                  >×</text>
                </g>
            ...
            {(md.slidingDoors || []).length > 0 && (
              <g onClick={(e) => { e.stopPropagation(); onItemClick?.(mRect.id, 'slidingDoor', md.slidingDoors[0].id); }}>
                ...
                <rect
                  x={mRect.x + mRect.width - 22} y={moduleTop + 14} width="18" height="18" rx="4"
                  fill="#ef4444" opacity="0.85"
                  onClick={(e) => { e.stopPropagation(); onRemoveObject?.(mRect.id, 'slidingDoors', md.slidingDoors[0].id); }}
                  style={{ cursor: 'pointer' }}
                />
                <text x={mRect.x + mRect.width - 13} y={moduleTop + 27} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold"
            ...
            {(md.slidingDoors || []).length === 0 && (md.doors || []).map((d, idx) => {
              ...
              return (
                <g key={d.id} onClick={(e) => { e.stopPropagation(); onItemClick?.(mRect.id, 'door', d.id); }}>
                  <rect x={dx + 2} y={moduleTop + 3} width={dw - 4} height={interiorH - 6}
                    fill="rgba(229,231,235,0.45)" stroke={active ? '#f97316' : '#8b6914'} strokeWidth={active ? 1.8 : 1.2} />
      ...
      {(draftState?.facadeItems || []).map((a) => {
        ...
        if (a.type === 'dim') {
          return (
            <g key={a.id} onClick={() => onAnnotationClick?.(a.id)} style={{ cursor: 'pointer' }}>
              <line x1={a.x1} y1={a.y1} x2={a.x2} y2={a.y2} stroke="#f97316" strokeWidth={sel ? 2.6 : 1.8}
        ...
        if (a.type === 'arrow') {
          return (
            <g key={a.id} onClick={() => onAnnotationClick?.(a.id)} style={{ cursor: 'pointer' }}>
              <line x1={a.x1} y1={a.y1} x2={a.x2} y2={a.y2} stroke="#ef4444" strokeWidth={sel ? 2.6 : 2} markerEnd="url(#annArrowRed)" />
        ...
        return (
          <g key={a.id} onClick={() => onAnnotationClick?.(a.id)} style={{ cursor: 'pointer' }}>
            <rect x={a.x1 - 42} y={a.y1 - 14} width="84" height="22" rx="8" fill="rgba(255,255,255,0.88)" stroke={sel ? '#f97316' : '#22d3ee'} />
      ...
      <text
        ...
        textAnchor="middle" fill="#ef4444" fontSize="12" fontWeight="700"
      >{draftState?.cabinetDims?.height || '?'} cm</text>

      {dragPreview && (
        <line x1={dragPreview.x1} y1={dragPreview.y1} x2={dragPreview.x2} y2={dragPreview.y2}
          stroke={dragPreview.type === 'dim' ? '#f97316' : '#ef4444'} strokeWidth="2" strokeDasharray="4 3" />
      )}
