    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-950/50 overflow-hidden h-full flex flex-col" ref={wrapRef}>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        {(viewMode === 'scan' || viewMode === 'split') && (
          <div className={`relative overflow-hidden bg-slate-950 ${viewMode === 'split' ? 'w-1/2 border-r border-slate-700' : 'w-full'}`}>
            <div className="absolute top-2 left-3 text-[10px] text-slate-500 z-10 bg-slate-900/70 px-2 py-0.5 rounded pointer-events-none">
              SCAN ORIGINAL
            </div>
            {image
              ? <img
        ...
        {(viewMode === 'facade' || viewMode === 'split') && (
          <div
            className={`relative overflow-auto bg-slate-950/30 ${viewMode === 'split' ? 'w-1/2' : 'w-full'}`}
            style={{ cursor }}
