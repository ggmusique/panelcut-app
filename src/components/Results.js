...
function PanelSVG({ panel, panelW, panelH, kerf, colorMap }) {
  ...
  return (
    <div className="relative group">
      ...
      <svg viewBox={`0 0 ${SVG_W + MARGIN_R} ${SVG_H}`} className="relative w-full h-auto bg-[#0a0a0a] rounded-xl border border-white/10 shadow-2xl" style={{ display: 'block' }}>
        <defs>
          <pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(255,255,255,0.03)" strokeWidth="0.5" /></pattern>
          <pattern id="hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="6" stroke="rgba(239,68,68,0.25)" strokeWidth="2" /></pattern>
        </defs>
        ...
        {panel.placed.map((p, i) => {
          ...
          const px = (p.x || 0) * sx, py = (p.bandY || 0) * sy, pw = p.l * sx - 1, ph = p.h * sy - 1;
          return (<g key={i}>
        ...
        {(() => {
          ...
          for (const h of hBands) {
            const cy2 = h.pos * sy, n = num++;
            items.push(<g key={`h-${n}`}>
              <line x1={0} y1={cy2.toFixed(1)} x2={SVG_W} y2={cy2.toFixed(1)} stroke="#f59e0b" strokeWidth="1.5" strokeDasharray="6,3" />
              <text x={SVG_W+14} y={(cy2+4).toFixed(1)} fontSize="9" fontWeight="bold" fill="#f59e0b" fontFamily="monospace">{h.posCm}</text>
              <rect x={0} y={(cy2-9).toFixed(1)} width="18" height="14" rx="3" fill="#f59e0b" />
              <text x="9" y={(cy2+3).toFixed(1)} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#000" fontFamily="sans-serif">{n}</text>
            </g>);
            allBands.filter(c => c.orientation === 'vertical' && c.bandKey === h.bandKey).sort((a,b)=>(a.pos||0)-(b.pos||0)).forEach(v => {
              const cx2 = v.pos * sx, top = (v.bandY||0)*sy, nv = num++;
              items.push(<g key={`v-${nv}`}>
                <line x1={cx2.toFixed(1)} y1={top.toFixed(1)} x2={cx2.toFixed(1)} y2={(top+(v.bandH||0)*sy).toFixed(1)} stroke="#3b82f6" strokeWidth="1.5" strokeDasharray="5,3" opacity="0.8" />
                <rect x={(cx2-9).toFixed(1)} y={top.toFixed(1)} width="18" height="14" rx="3" fill="#3b82f6" />
                <text x={cx2.toFixed(1)} y={(top+10).toFixed(1)} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#fff" fontFamily="sans-serif">{nv}</text>
              </g>);
            });
          }
          return items;
...
function CutList({ panel }) {
  ...
  for (const h of hCuts) {
    ...
    bandCuts.filter(c => c.orientation === 'vertical' && c.bandKey === h.bandKey).sort((a,b)=>(a.pos||0)-(b.pos||0)).forEach(v => {
      const vPieces = panel.cuts.filter(pc => pc.type === 'piece' && pc.bandKey === v.bandKey && (pc.x||0) >= (v.pos||0));
        ...
        return (
          <div key={cut.num} style={{ display:'flex', alignItems:'flex-start', gap:10, background: isH?'rgba(245,158,11,0.08)':'rgba(59,130,246,0.08)', border:`1px solid ${isH?'rgba(245,158,11,0.2)':'rgba(59,130,246,0.2)'}`, borderRadius:8, padding:'8px 10px' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {cut.pieces.length > 0 && <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>{cut.pieces.map((pc,pi) => <span key={pi} style={{ fontSize:10, color:'#94a3b8', background:'rgba(255,255,255,0.05)', padding:'2px 6px', borderRadius:4, fontFamily:'monospace' }}>{pc.name} {pc.lCm}×{pc.hCm}cm{pc.rotated?' ↺':''}</span>)}</div>}
            </div>
          </div>
        );
      })}
    ...
    <div className="flex bg-[#111] border border-white/5 rounded-xl p-1 gap-1">
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)}
          className={'flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold transition-all ' + (active === t.id ? 'bg-orange-600 text-white shadow-lg shadow-orange-900/30' : 'text-slate-400 hover:text-white')}>
          {t.icon}{t.label}
        </button>
      ))}
    </div>
...
/**
 * Reconstruit le model3D depuis les pièces optimisées.
 * Les pièces dans project.pieces sont en MM → on divise par 10 pour avoir des CM.
 * CabinetPreview3D attend des CM (il fait /100 en interne pour Three.js).
 */
function buildModel3DFromProject(project) {
  const pieces = project.pieces || [];

  // Pièces en mm → cm
  const piecesInCm = pieces.map(p => ({
    name:   p.name,
    length: parseFloat(p.length) / 10,  // mm → cm
    height: parseFloat(p.height) / 10,  // mm → cm
    qty:    parseInt(p.qty, 10) || 1,
  }));

  // Largeur totale du meuble = pièce "Côté" ou max des largeurs
  const cotePiece = piecesInCm.find(p => p.name === 'Côté');
  const dessousPiece = piecesInCm.find(p => p.name === 'Dessous' || p.name === 'Dessus');

  // Hauteur = hauteur de la pièce Côté
  const furnitureH = cotePiece ? cotePiece.height : (project.furnitureHeight ?? 220);
  // Profondeur = largeur de la pièce Côté  
  const furnitureD = cotePiece ? cotePiece.length : (project.furnitureDepth ?? 60);
  // Largeur meuble = largeur des tablettes + 2 × épaisseur joue
  const T = 1.8;
  const furnitureW = dessousPiece ? (dessousPiece.length + T * 2) : (project.panel?.w ?? 244);

  // Modules = largeurs des tablettes (pièces Tablette M*)
  const modules = piecesInCm
    .filter(p => p.name.startsWith('Tablette'))
    .map(p => ({ width: p.length }));

  // Si pas de tablettes, on déduit depuis les séparations
  if (modules.length === 0) {
    const sep = piecesInCm.find(p => p.name === 'Séparation');
    const nbModules = sep ? sep.qty + 1 : 1;
    const moduleW = (furnitureW - T * (nbModules + 1)) / nbModules;
    for (let i = 0; i < nbModules; i++) modules.push({ width: moduleW });
  }

  return {
    dimensions: { width: furnitureW, height: furnitureH, depth: furnitureD },
    material:   { panelThickness: T },
    structure:  { modules },
  };
}
...
export default function Results({ t, results, project }) {
  ...
  // Modèle 3D reconstruit depuis les vraies pièces du projet
  const model3D = buildModel3DFromProject(project);
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center">
                ...
                <div className="w-full bg-white/10 h-1 rounded-full mt-2 overflow-hidden"><div className={`h-full rounded-full ${utilization >= 80 ? 'bg-green-500' : utilization >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${utilization}%` }} /></div>
              </div>
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center">
            ...
            <button onClick={() => setTab('3d')} className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm font-bold text-slate-300 flex items-center justify-center gap-2 transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#111] border border-white/5 rounded-xl p-4">
              ...
              <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                          {piecesInBand.map((pc, pi) => (
                            <div key={pi} className="flex items-center justify-between bg-[#0a0a0a] p-2 rounded border border-white/5 text-sm">
                              <div className="flex items-center gap-2"><span className="text-slate-500">→</span><span className="text-white font-medium">{pc.name}</span></div>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-slate-400 text-xs">{pc.lCm}×{pc.hCm}cm</span>
                                {pc.rotated && <span className="flex items-center gap-1 text-[10px] bg-purple-500/10 text-purple-400 px-1.5 py-0.5 rounded border border-purple-500/20"><RotateCw className="w-3 h-3" /></span>}
                              </div>
                            </div>
                          ))}
                        </div>
                ...
                {panel.cuts.filter(c => c.type === 'bande').length === 0 && <div className="text-center text-slate-500 py-10">Aucune coupe requise.</div>}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === '3d' && (
          <div className="space-y-4">
            <div className="bg-[#111] border border-white/5 rounded-xl p-4">
              ...
              <p className="text-xs text-slate-500 mt-3 text-center">Glissez pour faire tourner · Scroll pour zoomer</p>
            </div>
            <div className="bg-[#111] border border-white/5 rounded-xl p-4">
              <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Dimensions reconstruites depuis les pièces</h4>
              <div className="grid grid-cols-3 gap-3 text-center">
                <div><div className="text-lg font-black text-white">{Math.round(model3D.dimensions.width)}</div><div className="text-[10px] text-slate-500 uppercase">Larg. cm</div></div>
                <div><div className="text-lg font-black text-white">{Math.round(model3D.dimensions.height)}</div><div className="text-[10px] text-slate-500 uppercase">Haut. cm</div></div>
                <div><div className="text-lg font-black text-white">{Math.round(model3D.dimensions.depth)}</div><div className="text-[10px] text-slate-500 uppercase">Prof. cm</div></div>
              </div>
              <div className="mt-3 text-[10px] text-slate-600 text-center">{model3D.structure.modules.length} module(s) · épaisseur {model3D.material.panelThickness} cm</div>
            </div>
          </div>
        )}
