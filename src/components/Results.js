...
function PanelSVG({ panel, panelW, panelH, colorMap }) {
  ...
  return (
    <div className="relative group">
      ...
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="relative w-full h-auto bg-[#0a0a0a] rounded-xl border border-white/10 shadow-2xl" style={{ display: 'block' }}>
        <defs>
        ...
        {bandCuts.map((c, i) => {
          const isV = c.orientation === 'vertical';
          if (isV) {
            const cx = c.pos * sx;
            return (
              <g key={i}>
                <line x1={cx.toFixed(1)} y1={0} x2={cx.toFixed(1)} y2={SVG_H} stroke="#ef4444" strokeWidth="1.5" strokeDasharray="6,3" />
                <rect x={(cx-15).toFixed(1)} y="4" width="30" height="14" rx="2" fill="#000" fillOpacity="0.9" />
          ...
          return (
            <g key={i}>
              ...
              <rect x={(SVG_W-35).toFixed(1)} y={(cy-7).toFixed(1)} width="30" height="14" rx="2" fill="#000" fillOpacity="0.9" />
...
export default function Results({ t, results, project }) {
  ...
  /* ── Navigation panneaux (partagée entre onglets Visuel et Coupes) ── */
  const PanelNav = () => (
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-slate-200 pb-32 font-sans">
      ...
      {/* Barre onglets sticky sous le header */}
        ...
        {tab === 'resume' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center justify-center">
                <div className="text-[10px] text-slate-400 uppercase font-bold text-center mb-2">Panneaux</div>
                <div className="text-3xl font-black text-white">{results.summary.totalPanels}</div>
                <div className="text-[10px] text-orange-400 mt-1 font-medium">Nécessaires</div>
              </div>
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center justify-center">
                <div className="text-[10px] text-slate-400 uppercase font-bold text-center mb-2">Utilisation</div>
              ...
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center justify-center">
                <div className="text-[10px] text-slate-400 uppercase font-bold text-center mb-2">Chutes</div>
                <div className="text-3xl font-black text-red-400">{results.summary.wastePct}%</div>
                <div className="text-[10px] text-slate-500 mt-1 font-medium">Perdues</div>
              </div>
              <div className="bg-[#111] border border-white/5 p-4 rounded-xl flex flex-col items-center justify-center">
                <div className="text-[10px] text-slate-400 uppercase font-bold text-center mb-2">Coût</div>
                <div className="text-2xl font-black text-blue-400">{totalCost}€</div>
                <div className="text-[10px] text-slate-500 mt-1 font-medium">Matière</div>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(colorMap).map(([name, c]) => (
                  <div key={name} className="flex items-center gap-2 bg-[#0a0a0a] px-3 py-1.5 rounded-lg border border-white/5">
                    <div className="w-3 h-3 rounded-sm flex-shrink-0" style={{ background: c.fill, border: `1px solid ${c.stroke}`, boxShadow: `0 0 6px ${c.glow}` }}></div>
