...
const defaultModuleDetail = (drawerCount = 0) => ({
  ...
  traverseOnUprights: true,
  shelfSideClearanceMm: 4,
  shelfBackClearanceMm: 0,
  moduleFormula: '',
});
...
export default function SketchEditor({ image, scanImage, initialResult, apiKey, draft, onDraftChange, onComplete, onCancel, onSave }) {
  ...
  useEffect(() => {
    ...
    setModuleDetails((prev) => {
      return facadeModules.map((_, i) => ({
        ...
        traverseOnUprights: prev[i]?.traverseOnUprights ?? true,
        shelfSideClearanceMm: Math.max(0, toNum(prev[i]?.shelfSideClearanceMm, 4)),
        shelfBackClearanceMm: Math.max(0, toNum(prev[i]?.shelfBackClearanceMm, 0)),
        moduleFormula: typeof prev[i]?.moduleFormula === 'string' ? prev[i].moduleFormula : '',
        drawerHeights: (() => {
    ...
    return {
      ...
      modules: facadeModules.map((m, i) => {
        ...
        return {
          ...
          fabricationRules: {
            traverseOnUprights: details.traverseOnUprights ?? true,
            shelfSideClearanceMm: Math.max(0, toNum(details.shelfSideClearanceMm, 4)),
            shelfBackClearanceMm: Math.max(0, toNum(details.shelfBackClearanceMm, 0)),
            moduleFormula: typeof details.moduleFormula === 'string' ? details.moduleFormula.trim() : '',
          },
          rods: facadeItems
  ...
  const buildContextPrompt = useCallback(() => {
    ...
    facadeModules.forEach((m, i) => {
      ...
      const shelfSideClearance = Math.max(0, toNum(det.shelfSideClearanceMm, 4));
      const shelfBackClearance = Math.max(0, toNum(det.shelfBackClearanceMm, 0));
      ...
      ctx += `      regles: traverse_sur_montants=${det.traverseOnUprights ?? true ? 'oui' : 'non'}  jeu_tablette_GD=${shelfSideClearance}mm  jeu_tablette_fond=${shelfBackClearance}mm\n`;
      if (det.moduleFormula?.trim()) ctx += `      formule_module="${det.moduleFormula.trim()}"\n`;
      if (nbDrawers > 0) {
  ...
  const applyPresetToSelectedModule = (preset) => {
    setModuleDetails((prev) => prev.map((d, i) => {
      if (i !== selectedModuleIdx) return d;
      const base = {
        ...defaultModuleDetail(facadeModules[i]?.drawers || 0),
        ...(d || {}),
      };
      if (preset === 'sans_fond') return { ...base, hasBack: false };
      if (preset === 'traverse_sur_montants') return { ...base, traverseOnUprights: true };
      if (preset === 'tablette_4mm') return { ...base, shelfSideClearanceMm: 4 };
      return base;
    }));
  };

  const copySelectedRulesToAllModules = () => {
    const source = moduleDetails[selectedModuleIdx] || defaultModuleDetail(facadeModules[selectedModuleIdx]?.drawers || 0);
    setModuleDetails((prev) => prev.map((d, i) => ({
      ...defaultModuleDetail(facadeModules[i]?.drawers || 0),
      ...(d || {}),
      hasBack: source.hasBack ?? true,
      traverseOnUprights: source.traverseOnUprights ?? true,
      shelfSideClearanceMm: Math.max(0, toNum(source.shelfSideClearanceMm, 4)),
      shelfBackClearanceMm: Math.max(0, toNum(source.shelfBackClearanceMm, 0)),
      moduleFormula: typeof source.moduleFormula === 'string' ? source.moduleFormula : '',
    })));
  };
    ...
    >
      ...
      {facadeModules.length > 0 && (
        <div className="bg-slate-900/95 border-b border-slate-700 px-3 py-2 flex flex-wrap items-center gap-3 text-xs">
          ...
          <span className="text-cyan-300 font-bold">Règles module IA:</span>
          <div className="flex items-center gap-1">
            <button type="button" onClick={() => applyPresetToSelectedModule('sans_fond')} className="px-2 py-1 rounded border border-slate-600 bg-slate-800 text-slate-200 hover:bg-slate-700">Sans fond</button>
            <button type="button" onClick={() => applyPresetToSelectedModule('traverse_sur_montants')} className="px-2 py-1 rounded border border-slate-600 bg-slate-800 text-slate-200 hover:bg-slate-700">Traverse sur montants</button>
            <button type="button" onClick={() => applyPresetToSelectedModule('tablette_4mm')} className="px-2 py-1 rounded border border-slate-600 bg-slate-800 text-slate-200 hover:bg-slate-700">Tablette -4mm / côté</button>
            <button type="button" onClick={copySelectedRulesToAllModules} className="px-2 py-1 rounded border border-cyan-500/50 bg-cyan-900/30 text-cyan-200 hover:bg-cyan-800/40">Copier règles → tous modules</button>
          </div>
          <label className="flex items-center gap-1 text-slate-200">
            <input
              type="checkbox"
              checked={moduleDetails[selectedModuleIdx]?.traverseOnUprights ?? true}
              onChange={(e) => setModuleDetails(prev => prev.map((d, i) => i === selectedModuleIdx ? { ...d, traverseOnUprights: e.target.checked } : d))}
            />
            Traverses sur montants
          </label>
          <label className="text-slate-300">
            ...
            <input
              ...
              min="0"
              step="0.5"
              value={moduleDetails[selectedModuleIdx]?.shelfSideClearanceMm ?? 4}
              onChange={(e) => setModuleDetails(prev => prev.map((d, i) => i === selectedModuleIdx ? { ...d, shelfSideClearanceMm: Math.max(0, toNum(e.target.value, 4)) } : d))}
              className="w-16 ml-1 px-1 py-0.5 bg-slate-800 border border-slate-600 rounded"
            />
          </label>
          <label className="text-slate-300">
            Jeu tablette fond (mm)
            <input
              type="number"
              min="0"
              step="0.5"
              value={moduleDetails[selectedModuleIdx]?.shelfBackClearanceMm ?? 0}
              onChange={(e) => setModuleDetails(prev => prev.map((d, i) => i === selectedModuleIdx ? { ...d, shelfBackClearanceMm: Math.max(0, toNum(e.target.value, 0)) } : d))}
              className="w-16 ml-1 px-1 py-0.5 bg-slate-800 border border-slate-600 rounded"
            />
          </label>
          <label className="text-slate-300 flex items-center gap-1">
            ...
            <input
              type="text"
              value={moduleDetails[selectedModuleIdx]?.moduleFormula ?? ''}
              onChange={(e) => setModuleDetails(prev => prev.map((d, i) => i === selectedModuleIdx ? { ...d, moduleFormula: e.target.value } : d))}
              placeholder="ex: 2 traverses, tablette libre, sans fond"
              className="w-64 px-1 py-0.5 bg-slate-800 border border-slate-600 rounded"
            />
          </label>
        </div>
      )}
