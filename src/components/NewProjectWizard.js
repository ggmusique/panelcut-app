...
export default function NewProjectWizard({ t, project, onChange, onGoScan, onGoManual, onCancel }) {
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      // Extraire le base64 pur (sans header) pour l'envoi API si nécessaire, 
      // mais ici on va envoyer en FormData directement pour éviter les erreurs de taille JSON
      ...
      const response = await fetch(apiUrl, {
        method: 'POST',
        body: formData,
        // Ne pas définir Content-Type manuellement, le navigateur le fait pour FormData
      });
      ...
      // 3. Appeler le callback avec le résultat ET l'image complète (header inclus)
      if (onGoScan) {
...
export default function NewProjectWizard({ t, project, onChange, onGoScan, onGoManual, onCancel }) {
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      const API_URL = process.env.REACT_APP_SCAN_API_URL || 'http://localhost:3001';
      ...
      if (!response.ok) {
        ...
        throw new Error(err.error || 'Erreur serveur lors du scan');
      }
      const scanResult = await response.json();
      if (onGoScan) onGoScan(scanResult, `${file.type};base64,${base64}`);
    } catch (err) {
      console.error('Scan error:', err);
      alert(`❌ Échec du scan : ${err.message}`);
    } finally {
