...
import CabinetPlan2D         from './components/CabinetPlan2D';
import CabinetPlan3D         from './components/CabinetPlan3D';
...
const FACADE_TABS = [
  { id: 'sketch',   label: '📐 Croquis',     emoji: '📐' },
  { id: 'plan2d',   label: '📋 Plan 2D',     emoji: '📋' },
  { id: 'iso3d',    label: '🧊 Vue 3D Iso',  emoji: '🧊' },
  { id: 'realistic',label: '🖼️ Vue Réaliste', emoji: '🖼️' },
];
...
const LS_FACADE_TAB = 'pc_facade_tab';
...
function lsClear() {
  [LS_SCREEN, LS_PROJECT, LS_RESULTS, LS_FACADE_TAB].forEach(k => localStorage.removeItem(k));
}
...
export default function App() {
  ...
  const [screen,  setScreenRaw]  = useState(() => {
    ...
    // FACADE_REALISTIC n'existe plus — on redirige vers FACADE
    const valid = Object.values(SCREENS);
    return valid.includes(stored) ? stored : SCREENS.LANDING;
  });
  ...
  // Onglet actif de la page Façade
  const [facadeTab, setFacadeTabRaw] = useState(() => lsGet(LS_FACADE_TAB, 'sketch'));
  const setFacadeTab = (t) => { setFacadeTabRaw(t); lsSet(LS_FACADE_TAB, t); };
  ...
  const goBack = () => {
    if      (screen === SCREENS.FACADE)  setScreen(SCREENS.RESULTS);
  ...
  const handleScanComplete = (scanResult, scanImageBase64) => {
    ...
    setProject(prev => ({
      ...prev, pieces,
      cabinet:    scanResult.cabinet    || null,
  ...
  const handleRefinementComplete = (newScanResult) => {
    ...
    const rawCabinet = newScanResult.cabinet || newScanResult.result?.cabinet || project.cabinet;
  ...
  const showBack = [SCREENS.PIECES, SCREENS.RESULTS, SCREENS.FACADE].includes(screen);
  ...
  else if (screen === SCREENS.FACADE)  { headerTitle = 'Plan & Façade'; steps = [{ label: 'Panneau', active: true }, { label: 'Pièces', active: true }, { label: 'Façade', active: true }]; }

  const hasHeader = ![SCREENS.AUTH, SCREENS.SKETCH, SCREENS.LANDING, SCREENS.WIZARD, SCREENS.HISTORY].includes(screen);
  const hasSteps  = steps.length > 0;

  // ── Cabinet normalisé passé à tous les composants de vue ─────────────────────
  const normalizedCabinet = project.cabinet ? reconstructModulesFromFlat(project.cabinet) : null;
  ...
  return (
    <div className="app min-h-screen bg-[#0f1620] text-slate-200 font-sans dark:bg-slate-950 dark:text-slate-100 transition-colors duration-300">
      ...
      {screen === SCREENS.SKETCH  && (
        <SketchEditor image={project.scanImage} initialResult={project.scanResult} apiKey={apiKey} onComplete={handleRefinementComplete} onCancel={() => setScreen(SCREENS.PIECES)} />
      )}
      ...
      {![SCREENS.LANDING, SCREENS.WIZARD, SCREENS.HISTORY, SCREENS.SKETCH].includes(screen) && (
        <>
          {hasHeader && (
            <header className="sticky top-0 z-40 bg-[#0f1620]/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-white/10 shadow-lg h-16 flex items-center justify-between px-4 md:px-8 gap-2 transition-colors duration-300">
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                {showNext && (
                  <button onClick={canGoNext ? goNext : undefined}
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {canAnnotate && (
                  <button onClick={() => setScreen(SCREENS.SKETCH)}
                ...
                {showSave && !saving && (
                  <button onClick={handleSave} className="p-1.5 rounded-lg text-slate-400 hover:text-green-400 hover:bg-white/10 transition-colors"><Disc className="w-4 h-4" /></button>
                )}
                ...
                {user && (
                  <button onClick={handleSignOut} className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-white/10 transition-colors"><LogOut className="w-4 h-4" /></button>
                )}
              </div>
            </header>
          )}
          ...
          <main className={'w-full px-4 md:px-8 mb-20 ' + (hasHeader ? 'mt-4' : 'mt-0')}>
                ...
                : (
                  <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 p-8">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm text-slate-400">Les données sont manquantes ou corrompues.</p>
                    </div>
            ...
            {/* ── PAGE FAÇADE : 4 ONGLETS partageant tous normalizedCabinet ── */}
            {screen === SCREENS.FACADE && (
              <div className="max-w-5xl mx-auto">

                {/* Barre d'onglets */}
                <div className="flex gap-1 mb-6 bg-[#070e1a] p-1.5 rounded-xl border border-white/10">
                  {FACADE_TABS.map(tab => (
                    <button
                      key={tab.id}
                ...
                {/* Contenu de l'onglet actif — tous reçoivent normalizedCabinet */}
                {facadeTab === 'sketch' && (
                  <CabinetElevationFront
                    cabinet={normalizedCabinet}
                    name={project.name || 'Meuble'}
                  />
                )}
                ...
                {facadeTab === 'realistic' && (
                  <RealisticFacadeViewer
                    cabinet={normalizedCabinet}
                    projectName={project.name || 'Meuble'}
                  />
                )}
