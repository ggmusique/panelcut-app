    ...
    <div className="app min-h-screen bg-[#0f1620] text-slate-200 font-sans">
      {hasHeader && (
        <header className="sticky top-0 z-40 bg-[#0f1620]/95 backdrop-blur-md border-b border-white/10 shadow-lg h-16 flex items-center justify-between px-4 md:px-8 gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <button onClick={toggleLang} className="px-2 py-1 text-slate-400 hover:text-white border border-white/10 hover:border-white/30 rounded-lg text-[11px] font-bold transition-colors">{langOverride === 'fr' ? 'EN' : 'FR'}</button>
            {user && (<button onClick={handleSignOut} className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-white/10 transition-colors"><LogOut className="w-4 h-4" /></button>)}
          </div>
        </header>
      )}
      ...
      <main className={'w-full px-4 md:px-8 mb-20 ' + (hasHeader ? 'mt-4' : 'mt-0')}>
        ...
        {screen === SCREENS.PROJECTS && (
          <ProjectsScreen user={user} onNew={() => startNew(devisNum)} onLoad={handleLoadProject} onScanComplete={handleScanComplete} onPlans={() => setScreen(SCREENS.PLANS)} />
        )}
        {screen === SCREENS.SCAN     && (
          <Scanner t={tr} onClose={() => setScreen(SCREENS.PROJECTS)} onPiecesDetected={(pieces) => {
            const projectName = 'Plan du ' + new Date().toLocaleDateString('fr-FR');
            const dNum = 'DV-' + new Date().getFullYear() + '-' + String(Math.floor(Math.random() * 9000) + 1000);
            setProject({ ...DEFAULT_PROJECT, name: projectName, devisNum: dNum, pieces });
            setResults(null);
        ...
        {screen === SCREENS.PLANS    && (
          <PlansScreen project={project} session={{ user }} supabase={supabase} t={tr} onBack={() => setScreen(SCREENS.PROJECTS)} onScan={() => setScreen(SCREENS.SCAN)} />
        )}
        ...
        {screen === SCREENS.RESULTS  && results && (
          <Results
            ...
            onSave={({ svgData, title, type, metadata }) => {
              ...
              supabase.from('plans').insert({ project_id: project?.id, user_id: user?.id, svg_data: svgData, title, type, metadata });
            }}
          />
        )}
      </main>
